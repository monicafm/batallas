...
public class Tablero {
	...
	private void inicializarTablero() {
		...
		// batalla.tablero posiciones en blanco
	...
	public void anotarDisparoEnTableroBarcos(Coordenada c, boolean b) {
		if (b == true) {
			tablabarcos[c.getCoordenadaX()][c.getCoordenadaY()] = 'T';
		} else {
			tablabarcos[c.getCoordenadaX()][c.getCoordenadaY()] = 'x';
		}
	}
	...
	public boolean devolverHundido(Coordenada c, Tablero tableroDisparos) {
		...
		if (tabladisparos[c.getCoordenadaX()][c.getCoordenadaY()] == estadoCasilla.H) {// OJO
																						// comprobar
																						// tableroDisparos
																						// e
																						// if
	...
	public void imprimirTableroBarcos() {
		...
		for (int i = 0; i < alto; i++) {
			...
			}// para mostrar batalla.tablero 10x10
			else {
				System.out.print((i + 1) + "   ");
			}// para mostrar batalla.tablero 10x10
			for (int j = 0; j < ancho; j++) {
				System.out.print(tablabarcos[i][j]);
				if (j != tablabarcos[i].length - 1)
					System.out.print("  ");
			}
			System.out.println();
		}
